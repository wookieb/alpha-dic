...
describe("TransformArg", () => {
	const VALUE = "foo";
	const DEFINITION_1 = Definition.create().useValue("val");
	...
	it("applies transform function on arg result", async () => {
		...
		expect(arg.getDependentServices(container)).toEqual([]);

		await expect(arg.getArgument(container)).resolves.toEqual(VALUE);
	...
	it("returns the same deps as arg", () => {
		...
		expect(arg.getDependentServices(container)).toEqual([DEFINITION_1]);
	});
});
