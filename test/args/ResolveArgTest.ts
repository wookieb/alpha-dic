...
describe("ResolveArg", () => {
	const DEFINITION_1 = Definition.create().useValue(1);

	const DEFINITION_2 = Definition.create().useValue(2);
		...
		it.each<[string, any, any, Definition[]]>([
			...
			[
				"object with nested properties",
				{
					...
					arrOf: [ARGUMENT_FOO, ARGUMENT_BAR],
					deps: [ARGUMENT_FOO_WITH_DEPS],
			...
			["ignores primitive: string", "foo", "foo", []],
			["ignores primitive: undefined", undefined, undefined, []],
			[
			...
			["ignores primitive: number", 10, 10, []],
		])("case %s", async (_, value, expected, dependencies) => {
			const arg = new ResolveArg(value);
			const funcArg = new ResolveArg(() => value);

			const container = create();
			await expect(arg.getArgument(container)).resolves.toEqual(expected);

			await expect(funcArg.getArgument(container)).resolves.toEqual(expected);

			expect(arg.getDependentServices(container)).toEqual(dependencies);

			expect(funcArg.getDependentServices(container)).toEqual(dependencies);
		});
	});
});
