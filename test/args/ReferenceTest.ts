...
describe("Reference", () => {
	...
	beforeEach(() => {
		...
		container.definition("D").useValue(serviceD).markType(D);
	});
	...
	describe("multi", () => {
		...
		it("by annotation - nothing found", async () => {
			...
			expect(ref.getDependentServices(container)).toHaveLength(0);

			expect(await ref.getArgument(container)).toHaveLength(0);
		});
