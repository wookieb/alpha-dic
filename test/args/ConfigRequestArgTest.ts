...
describe("ConfigRequestArg", () => {
	...
	it("is ContainerArg", () => {
		expect(new ConfigRequestArg(PATH)).toBeInstanceOf(ContainerArg);
	});
	...
	it("ConfigRequestArg.create", () => {
		expect(ConfigRequestArg.create("path")).toEqual(new ConfigRequestArg("path"));

		expect(ConfigRequestArg.create("path", "withDefaultValue")).toEqual(
			new ConfigRequestArg("path", "withDefaultValue")
		);

		expect(ConfigRequestArg.create("path", undefined)).toEqual(
			new ConfigRequestArg("path", undefined)
		);
	});
	...
	describe("as argument", () => {
		...
		it("resolves to config", () => {
			...
			return expect(REQUEST.getArgument(container)).resolves.toEqual("bar");
		});

		it("has no dependencies", () => {
			container.addMiddleware(MIDDLEWARE);
			expect(REQUEST.getDependentServices(container)).toEqual([]);
		});
	});
});
