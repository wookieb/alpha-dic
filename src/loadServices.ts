...
function createExtensionsPredicate(extensions: string[], defaultResult: boolean): Predicate<string> {
...
export function loadServices(container: Container, opts?: loadServices.Options) {
    ...
    const patterns = opts?.patterns ? (is.arr(opts.patterns) ? opts.patterns : [opts.patterns]) : ['**/*'];
