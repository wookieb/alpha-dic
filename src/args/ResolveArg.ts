...
export class ResolveArg<T extends object> extends ContainerArg<ResolveArg.Resolved<T>> {
	...
	private async transformObject<T>(
		result: T,
		container: Container
	): Promise<ResolveArg.Resolved<T>> {
		...
		if (is.plainObject(result)) {
			...
			for (const [key, value] of Object.entries(result)) {
				newObject[key] = isOmitableType(value)
					? value
					: await this.transformObject(value, container);
			}
			return newObject;
		}
	...
	private collectContainerArgsFromResult(result: T): ContainerArg[] | undefined {
		...
		if (is.plainObject(result)) {
			return Object.values(result).reduce(this.reducer, []);
		}
	}
	...
	getDependentServices(container: Container): Definition | Definition[] {
		...
		if (args) {
			return args.reduce<Definition[]>((acc, current) => {
				...
				return acc.concat(Array.isArray(deps) ? deps : [deps]);
			}, []);
		}
		return [];
	}
...
export namespace ResolveArg {
	export type Resolved<T> =
		T extends ContainerArg<infer U>
			? U
